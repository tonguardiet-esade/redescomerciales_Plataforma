...
const contentData: Record<number, { title: string, objective: string, content: string[] }> = {
  [UserLevel.PRESCRIPTOR]: {
    title: "Las soluciones del ecosistema RedesComerciales.ai",
    objective: "Comprender las diferentes plataformas especializadas que forman el ecosistema.",
    content: [
      "El ecosistema RedesComerciales.ai está formado por diferentes plataformas diseñadas para abordar distintas necesidades dentro del proceso de ventas y formación.",
      "SalesAcademy: Formación continua para colaboradores. LeadHub: Herramientas para la identificación y cualificación de oportunidades.",
      "NetworkManager: Gestión integral de la red de colaboradores y seguimiento de objetivos.",
      "Todas las soluciones comparten tecnología avanzada, IA supervisada por expertos y entornos seguros para gestionar información sensible."
    ]
  },
...
const quizData: Record<number, { title: string, questions: { question: string, options: { text: string, correct: boolean }[] }[] }> = {
  [UserLevel.PRESCRIPTOR]: {
    ...
    questions: [
      {
        question: "¿Qué solución del ecosistema se centra en la formación?",
        options: [
          { text: "SalesAcademy", correct: true },
          { text: "LeadHub", correct: false },
          { text: "NetworkManager", correct: false }
        ]
      },
      {
        question: "¿Qué herramienta está orientada a la gestión de la red?",
        options: [
          { text: "SalesAcademy", correct: false },
          { text: "LeadHub", correct: false },
          { text: "NetworkManager", correct: true }
        ]
      },
      {
        question: "¿Qué tienen en común las soluciones del ecosistema RedesComerciales.ai?",
        options: [
          { text: "Son gratuitas", correct: false },
          { text: "Están basadas en tecnología e IA supervisada", correct: true },
          { text: "Garantizan ventas sin esfuerzo", correct: false }
        ]
      }
    ]
  },
...
const Module2 = () => {
          ...
          <div className="max-w-2xl mx-auto px-4 py-12 text-center animate-fade-in">
              ...
              <div className={`p-10 md:p-16 rounded-[3rem] shadow-xl border-2 ${passed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                  ...
                  <p className="text-gray-600 mb-10 font-bold leading-relaxed">
                      {passed 
                        ? `Excelente. Has demostrado conocer perfectamente las soluciones del ecosistema RedesComerciales.ai.`
                        : `Parece que hay conceptos de las soluciones del ecosistema que no han quedado claros. Es vital dominarlos para informar correctamente.`}
                  </p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <ChatAssistant context={`Módulo 2: Las soluciones del ecosistema RedesComerciales.ai`} />
    </div>
  );
};
