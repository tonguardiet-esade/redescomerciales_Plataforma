...
const contentData: Record<number, { title: string, objective: string, content: string[] }> = {
  [UserLevel.PRESCRIPTOR]: {
    title: "Para quién es RedesComerciales.ai",
    objective: "Identificar los perfiles de organizaciones que pueden beneficiarse del ecosistema de formación y ventas.",
    content: [
      "RedesComerciales.ai está pensado para organizaciones que buscan escalar sus ventas a través de redes de colaboradores.",
      "Entre los perfiles indicados se encuentran: pymes, empresas tecnológicas, grandes empresas, consultoras, asesorías, asociaciones y clústers.",
      "Es especialmente útil para quienes quieren transformar a sus colaboradores en una fuerza de ventas activa y estructurada.",
      "RedesComerciales.ai no está pensado para ventas individuales sin estructura. El papel del colaborador es clave para el crecimiento."
    ]
  },
...
const quizData: Record<number, { title: string, questions: { question: string, options: { text: string, correct: boolean }[] }[] }> = {
  [UserLevel.PRESCRIPTOR]: {
    ...
    questions: [
      {
        question: "¿Qué perfil encaja mejor con RedesComerciales.ai?",
        options: [
          { text: "Particular buscando financiación personal", correct: false },
          { text: "Empresa interesada en escalar sus ventas mediante redes", correct: true },
          { text: "Autónomo sin actividad comercial", correct: false }
        ]
      },
      {
        question: "¿Quién puede beneficiarse del ecosistema RedesComerciales.ai?",
        options: [
          { text: "Solo grandes corporaciones", correct: false },
          { text: "Empresas y organizaciones que buscan redes de ventas activas", correct: true },
          { text: "Cualquier persona física sin objetivos comerciales", correct: false }
        ]
      },
      {
        question: "¿Para quién NO está pensado RedesComerciales.ai?",
        options: [
          { text: "Pymes", correct: false },
          { text: "Asociaciones", correct: false },
          { text: "Ventas individuales sin estructura", correct: true }
        ]
      }
    ]
  },
          ...
          <div className="max-w-2xl mx-auto px-4 py-12 text-center animate-fade-in">
              ...
              <div className={`p-10 md:p-16 rounded-[3rem] shadow-xl border-2 ${passed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                  ...
                  <p className="text-gray-600 mb-10 font-bold leading-relaxed">
                      {passed 
                        ? `Excelente. Has demostrado saber identificar perfectamente el perfil de cliente ideal.`
                        : `Parece que hay dudas sobre quién puede beneficiarse de RedesComerciales.ai. Es fundamental tener claro el perfil de cliente para ser efectivo.`}
                  </p>
                  <div className="flex flex-col gap-4 justify-center items-center">
                    {passed ? (
                        <button onClick={() => navigate('/portal')} className="px-16 py-5 bg-brand-primary text-white font-black rounded-2xl uppercase tracking-widest shadow-lg hover:scale-105 transition-all">Volver al Panel</button>
                    ) : (
